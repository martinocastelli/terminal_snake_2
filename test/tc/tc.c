...
void tc_get_pressed_keys(tc_keyboard_input_d* keyboard_input) {
	...
	if(keyboard_input->normal_input != NULL) {
		free(keyboard_input->normal_input);
	} else {
		printf("porcodio 1\n");
	}
	keyboard_input->normal_input = (char*)malloc((keyboard_input->normal_input_size + 1)  * sizeof(char));
	if(keyboard_input->escape_input != NULL) {
		free(keyboard_input->escape_input);
	} else {
		printf("porcodio 2\n");
	}
	keyboard_input->escape_input = (tc_escape_sequence_input_e*)malloc((keyboard_input->escape_input_size + 1)  * sizeof(tc_escape_sequence_input_e));
	...
	keyboard_input->escape_input[keyboard_input->escape_input_size] = TC_NONE;
}
